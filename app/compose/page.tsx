      ...
      <div className="grid-2">
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-header-icon indigo">✏️</span>
            COMPOSE MESSAGE
          </div>
          ...
          <div className="form-group">
            ...
            {listError && <div className="alert alert-danger" style={{ marginTop: '8px', marginBottom: 0 }}>{listError}</div>}
          </div>
          ...
          <div className="form-group">
            ...
            <div style={{ marginBottom: '8px' }}>
              <select
                className="form-control"
                style={{ width: 'auto' }}
                onChange={e => setParseMode(e.target.value as 'bulk' | 'single')}
                value={parseMode}
              >
            ...
            />
            <button className="btn btn-outline btn-sm" style={{ marginTop: '8px', width: '100%' }} onClick={handleAddRecipients}>
              + ADD RECIPIENT{recipientInput.includes('\n') ? 'S' : ''}
            </button>
          </div>

          {recipients.length > 0 && (
            <div className="recipient-tags" style={{ marginBottom: '16px' }}>
              {recipients.map(r => (
            ...
            <>
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                  <button
                    className={`btn btn-sm ${!randomMode ? 'btn-primary' : 'btn-outline'}`}
                ...
                {randomMode && (
                  <div style={{ padding: '10px 12px', background: 'var(--bg-surface)', borderRadius: '6px', fontSize: '12px', color: 'var(--success)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="card">
            <div className="card-header">
              <span className="card-header-icon cyan">📧</span>
              LIVE PREVIEW
            </div>
            <div className="email-preview">
              <div className="email-preview-header">
                ...
                <div className="email-preview-subject">{subject || '<No subject>'}</div>
                {randomMode && (
          ...
          {result && (
            <div className="card">
              <div className="card-header">
                <span className="card-header-icon green">📡</span>
                TRANSMISSION REPORT
              </div>
              <div className="grid-2" style={{ marginBottom: '16px' }}>
                <div style={{ textAlign: 'center', padding: '16px', background: 'var(--bg-surface)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '28px', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 800, color: 'var(--success)' }}>{result.sent}</div>
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '4px' }}>SENT</div>
                </div>
                <div style={{ textAlign: 'center', padding: '16px', background: 'var(--bg-surface)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '28px', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 800, color: result.failed > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>{result.failed}</div>
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '4px' }}>FAILED</div>
                </div>
              </div>
              <div className="send-status">
                {result.results.map((r, i) => (
                  <div key={i} className="send-status-item">
                    ...
                    <span className={`badge ${r.success ? 'badge-success' : 'badge-danger'}`}>{r.success ? 'sent' : 'failed'}</span>
                  </div>
                ))}
              </div>
            </div>
