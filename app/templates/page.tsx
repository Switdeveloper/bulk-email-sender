  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 style={{ color: 'var(--accent)', margin: 0, fontSize: '20px' }}>📝 MESSAGE TEMPLATES</h2>
        ...
        {templates.map(t => (
          <div key={t.id} className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
              <div>
                <div style={{ color: 'var(--accent)', fontWeight: 'bold', fontSize: '15px' }}>{t.name}</div>
            ...
            <div style={{ color: 'var(--cyan)', fontSize: '12px', fontFamily: 'monospace', marginBottom: '8px' }}>
              SUBJECT: {t.subject}
            </div>
            ...
            {previewTemplate?.id === t.id && (
              <>
                ...
                <div>
                  ...
                  <div style={{ color: 'var(--cyan)', fontSize: '13px', marginBottom: '8px' }}>{t.subject}</div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="card-header">
              <div className="card-header-icon indigo">{editingId ? '✏️' : '➕'}</div>
              <span>{editingId ? 'EDIT TEMPLATE' : 'NEW TEMPLATE'}</span>
              <button onClick={() => { setShowForm(false); setEditingId(null) }}
                style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'var(--text-secondary)', fontSize: '20px', cursor: 'pointer' }}>×</button>
            </div>
